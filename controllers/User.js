...
exports.registerUser = async (req, res) => {
  try {
    ...
    let avatarData = {
      ...
      url: `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=random&color=fff&size=150`
    };
    ...
    if (req.file) {
      try {
        ...
        if (cloudinaryResult) {
          ...
          console.log('✅ Profile picture uploaded:', avatarData.url);
        }
      } catch (error) {
      ...
      console.log('ℹ️ No profile picture provided, using default avatar');
    }
    ...
    const user = await User.create({
      ...
      password,
      avatar: avatarData,
      isVerified: true,  // Auto-verify on registration (no email verification needed)
      isActive: true,
      authProvider: 'local'
    });
...
exports.firebaseLogin = async (req, res) => {
  try {
    ...
    if (decodedToken.email !== email) {
      return res.status(400).json({ message: 'Email does not match Firebase account' });
    }
    ...
    if (!user) {
      ...
      user = await User.create({
        name,
        email,
        password: crypto.randomBytes(16).toString('hex'), // Random password since Firebase handles auth
        avatar: {
          public_id: 'avatar_firebase_' + Date.now(),
          url: decodedToken.picture || `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=random&color=fff&size=150`
        },
      ...
      if (user.authProvider !== 'firebase') {
        user.authProvider = 'firebase'; // Mark as Firebase user
        await user.save();
        console.log('✅ User updated to Firebase auth provider');
      }
    }
    ...
    res.status(500).json({ success: false, message: 'Firebase login failed. Please try again.' });
  }
};
